...
import { Calendar as CalIcon, Eye, Pencil, Plus, Trash2 } from 'lucide-react';
...
export function TermineList() {
  ...
  const columns: Column<Termin>[] = [
    ...
    { key: 'grund', header: t('cols.grund'), sortValue: (r) => r.grund, cell: (r) => <Badge variant="secondary">{tc(`grund.${r.grund}`)}</Badge>, width: '170px' },
    { key: 'tech', header: t('cols.techniker'), sortValue: (r) => r.techniker, cell: (r) => r.techniker, width: '110px' },
    { key: 'status', header: t('cols.status'), sortValue: (r) => r.status, cell: (r) => <StatusBadge status={r.status} />, width: '140px' },
    {
      key: 'actions',
      header: '',
      align: 'right',
    ...
    <div>
      ...
      <Tabs defaultValue="calendar">
        ...
        <TabsContent value="calendar">
          <TermineCalendar
            ...
            onOpen={(x) => setModal({ kind: 'view', item: x })}
          />
        </TabsContent>
      ...
      <Dialog open={modal?.kind === 'view'} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-lg">
          ...
          {modal?.kind === 'view' ? <TerminView t={modal.item} /> : null}
        </DialogContent>
      </Dialog>
    </div>
  );
...
function TerminView({ t }: { t: Termin }) {
  const { t: tt } = useT('termine');
  const { t: tc } = useT('common');
  const kunde = customerById(t.customerId);
  const fzg = vehicleById(t.vehicleId);
  return (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="num text-lg font-semibold">
            <DateCell value={t.datum} /> · {t.von}–{t.bis}
          </div>
        </div>
